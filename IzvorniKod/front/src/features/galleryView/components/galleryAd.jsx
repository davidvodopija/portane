...
function GalleryAd({id, galleryId, onItemDeleted, actions = true}) {
    ...
    return (
        <div className="card wardrobe-item-card">
            <img
                ...
                onClick={() => {
                    (actions) ?
                        navigate(`/galleries/${galleryId}/listing/${id}`) : navigate(`/itemInfo/ad/${id}`)
                }}
            />

            <div className="card-body p-0 pb-2 ps-3 pe-1">
                {
                    (actions) ? <div className="actions-on-hover d-flex justify-content-end">
                        {
                            <HoverItemActions
                                shared={true}
                                onShare={null}
