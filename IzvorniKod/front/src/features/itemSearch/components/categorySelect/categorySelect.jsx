...
function CategorySelect({ mode }) {
	...
	const { user } = useAuth();
	const { searchTerm, entered, setTotalPages, page, formData, setFormData } =
	...
	return (
		<div className="d-flex container-div">
			<div className="mx-4 category-select p-3 mb-5 flex-wrap">
				...
				<form ref={formRef} className="custom-form">
					...
					<div className="mb-2">
						...
						<Select
							...
							name="categoryIds"
							isMulti
							options={codebooks["categories"].map(
								(category) => ({
									value: category.id,
									label: category.name,
								})
					...
					{formData.categoryIds &&
						formData.categoryIds.includes(6) && (
							<div className="mb-2">
								...
								<Select
									...
									isMulti
									options={codebooks["footwear-types"].map(
										(style) => ({
											value: style.id,
											label: style.name,
										})
			...
			{
				<div className="wardrobe-items-container d-flex flex-wrap ms-3 mx-lg-5 mx-md-2 mx-sm-0 mb-5">
					{itemList.length === 0 ? (
						<div></div>
						...
						itemList.map((item) => (
							<WardrobeItem
								...
								wardrobeId={
									mode === "myItems"
										? item.closetCustomComponent?.closetId
										: null
								}
								itemName={item.label}
								id={item.id}
